...
def image_to_text(path):
            ...
            texts= reader.readtext(image_arr, detail= 0)

            return texts, input_img

def extracted_text(texts):
            extrd_dict = {"NAME":[], "DESIGNATION":[], "COMPANY_NAME":[], "CONTACT":[], "EMAIL":[], "WEBSITE":[],
                            "ADDRESS":[], "PINCODE":[]}

            extrd_dict["NAME"].append(texts[0].title())
            extrd_dict["DESIGNATION"].append(texts[1].title())
...
st.markdown("<h1 style='text-align: center; color: DarkSlateBlue; font-size: 75px ;'>BizCardX: Extracting Business Card Data with OCR</h1>",
...
def setting_bg():
    st.markdown(f""" <style>.stApp {{
                        background:url("https://wallpapers.com/images/high/solid-project-building-a-decentralized-web-a89vppvgvchbnvhz.webp");
...
selected = option_menu(None, ["Home","Upload & Modify", "Delete"],
                       ...
                       styles={"nav-link": {"font-size": "25px", "text-align": "centre", "margin": "-2px", "--hover-color": "DarkSlateBlue"},
                               "icon": {"font-size": "25px"},
                               "container" : {"max-width": "3000px"},
                               "nav-link-selected": {"background-color": "DarkSlateBlue"}})

if selected == "Home":
        col1 , col2 = st.columns(2)
        with col1:
            st.write("<h1 style='text-align: center; color: DarkSlateGrey; font-size: 35px ;'>Bizcard is a Python application designed to extract information from business cards</h1>",
                unsafe_allow_html=True)
            st.write("<h1 style='text-align: center; color: DarkSlateGrey; font-size: 35px ;'>The main purpose of Bizcard is to automate the process of extracting key details from business card images, such as the name, designation, company, contact information, and other relevant data. By leveraging the power of OCR (Optical Character Recognition) provided by EasyOCR, Bizcard is able to extract text from the images.</h1>",
                unsafe_allow_html=True)
        
        with col2:
             st.image("Images/card.png")


elif selected == "Upload & Modify":
  ...
  if img is not None:
    ...
    if method == "Preview":
        ...
        if button_3:
            ...
            st.success(":red[MODIFYED SUCCESSFULLY]")
